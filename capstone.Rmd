...
stargazer(union(s1, s2), summary = FALSE, type="text")
...
A regression model was developed to predict accumulated GDUs based on seasonal, geographical, and environmental variables. While the model was based on specific conditions from five locations in the U.S. Midwest and should not be used to make predictions outside of this region, the code and steps could be readily applied to build a similar model using data from any other region of interest.

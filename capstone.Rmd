...
#### Correlations
The selection of model terms was based, in part, on practical concerns. Generally speaking, daily measurements for environmental variables, such as precip, sunlight, etc. will not be available for the season we wish to predict.

Fortunately, some of the remaining variables are expected to be good predictors of agdu. A very high correlation was seen between agdu and day_of_year and lower but potentially meaningful correlations between agdu and latitude and ersst. Although an annual trend for increasing temperatures was observed over the 20 year period of the dataset, correlation between year and agdu was relatively weak. Corrlation between agdu and latitude was also low. Unlike longitude, which measures distance in the North/South direction, latitude, which measures distance in the East/West direction, was not expected to be strongly associated with heat accumulation.

```{r}
cor(dplyr::select(envdat_inseason, agdu, day_of_yr, latitude, longitude, ersst, year))
```

Although correlation between agdu and day_of_yr is much higher than with any other dependent variable, the relationship is not linear.
